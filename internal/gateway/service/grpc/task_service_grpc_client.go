...
import (
	"context"
	clientService "github.com/distributedmarketplace/internal/gateway/service"
...
func (t *TaskServiceGrpcClient) CreateTask(ctx context.Context, task *model.Task) (*model.Task, error) {
	// Convert resources if any
	var resources []*pb.Resource
	if len(task.Resources) > 0 {
		resources = make([]*pb.Resource, len(task.Resources))
	...
	resp, err := t.client.CreateTask(ctx, req)
	if err != nil {
		return nil, err
	}
...
func (t *TaskServiceGrpcClient) DeleteTask(ctx context.Context, id string) error {
	req := &pb.DeleteTaskRequest{
		Id: id,
	}

	_, err := t.client.DeleteTask(ctx, req)
	return err
}
